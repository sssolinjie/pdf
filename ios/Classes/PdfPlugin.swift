...
public class PdfPlugin: NSObject, FlutterPlugin {
    static let invalid = NSNumber(value: -1)
    let dispQueue = DispatchQueue(label: "pdf")
    let documents = DocumentRepository()
    let pages = PageRepository()
    
    
  public static func register(with registrar: FlutterPluginRegistrar) {
    let channel = FlutterMethodChannel(name: "pdf", binaryMessenger: registrar.messenger())
    let instance = PdfPlugin()
